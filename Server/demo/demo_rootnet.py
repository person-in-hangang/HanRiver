...
from main.config_rootnet import cfg
from main.model_rootnet import get_pose_net
from common.utils.pose_utils_rootnet import process_bbox
from data.dataset_rootnet import generate_patch_image
from demo.demo_posenet import pose_demo
...
def root_demo(bbox, img_array) :
    ...
    print(bbox_list)
    print("original_img_height : ",original_img_height)
    print("original_img_width : ", original_img_width)

    # original
    # bbox_list = [[139.41, 102.25, 222.39, 241.57]]
    # test
    # bbox_list = [[270,170,65,185]]
    # test2
    # bbox_list = [[75,70,325,450]]
    ...
    for n in range(person_num):
        ...
        # print(str(n + 1) + 'person | ', 'Root joint x: ', int(vis_root[0]), ' Root joint y: ', int(vis_root[1]),
        #      ' Root joint depth: ' + str(root_3d[2]) + ' mm')

        depth = str(root_3d[2])

        #print("root_depth : " + depth)

        # posenet function
        rootnet_return = pose_demo(bbox_list, float(depth), img_array)

    return rootnet_return